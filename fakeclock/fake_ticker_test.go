...
func TestFakeTickerMultipleTickers(t *testing.T) {
	...
	t.Cleanup(ticker1.Stop)
	t.Cleanup(ticker2.Stop)

	// Receiving directly from ticker.C() makes it easy to miss events; use
	// counters instead.
	var count1 uint32
	var count2 uint32
	...
	eventually(t, time.Second, func() bool {
		return atomic.LoadUint32(&count1) == 1
	})
	eventually(t, time.Second, func() bool {
		return atomic.LoadUint32(&count2) == 1
	})

	// Ensure no extra ticks arrive shortly thereafter.
	deadline := time.Now().Add(5 * delta)
	for time.Now().Before(deadline) {
		if got := atomic.LoadUint32(&count1); got != 1 {
			t.Fatalf("ticker1 count=%d; want 1", got)
		}
		if got := atomic.LoadUint32(&count2); got != 1 {
			t.Fatalf("ticker2 count=%d; want 1", got)
		}
